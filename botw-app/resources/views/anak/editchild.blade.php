      ...
      <div class="card mb-3 btn-reveal-trigger">
        <div class="card-header position-relative min-vh-25 mb-8">
          ...
          <div class="avatar avatar-5xl avatar-profile shadow-sm img-thumbnail rounded-circle">
            <div class="h-100 w-100 rounded-circle overflow-hidden position-relative"> <img src="{{ asset('images/profile/' . $data -> user -> profile_picture) }}" width="200" alt="" data-dz-thumbnail="data-dz-thumbnail">
    ...
    <div class="card mb-3">
      ...
      <div class="card-body bg-light">
          <div class="col-lg-8">
            ...
            <input class="form-control" id="full-name" type="text" name="name" value="{{ $data -> user -> name }}">
          </div>
          <div class="col-lg-8">
            <label class="form-label" for="email">Email</label>
            <input class="form-control" id="email" type="text" name="email" value="{{ $data -> user -> email }}">
          </div>
          <div class="col-lg-8">
            <label class="form-label" for="email3">Kelompok Umur</label>
            <select class="form-select" aria-label="Default select example" name="gender">
              <option selected="" value="{{ $data -> gender }}">{{ $gender }}</option>
          ...
          <div class="col-lg-8">
            ...
            <select class="form-select" aria-label="Default select example" name="kelompok_umur_id">
              <option selected="" value="{{ $data -> kelompok_umur_id }}">{{ $kelompokumurskrg }}</option>
              @foreach ($data_ku as $item)
              <option value="{{ $item -> id }}">{{ $item -> ku_description }}</option>
              @endforeach
            </select>
          </div>
          <div class="col-lg-8">
            ...
            <select class="form-select" aria-label="Default select example" name="sponsor_anak_id">
              <option selected="" value="{{ $data -> sponsor_anak_id }}">{{ $sponsorskrg->sponsor->nama_sponsor }}</option>
